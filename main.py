...
if select == "Home":
    st.write("Welcome to Airbnb")

if select == "Data Analysis":
    price = st.checkbox("prices")
    availability = st.checkbox("Availability analysis")
    location_based = st.checkbox("location_analsis")
    geo = st.checkbox("geo")
    top = st.checkbox("top")
    if price:
...
if select == "About":   
    st.header(":red[***About the project***]")
    st.write("")
    st.write("")
    st.write("")
    st.write("")
